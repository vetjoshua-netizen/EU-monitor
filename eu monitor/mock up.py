...
with st.sidebar.expander("🌾 Self-Sufficiency"):
    ...
    if choice: current_selection = choice
with st.sidebar.expander("🐄 Self-sufficiency per animal category "):
...
st.subheader(f"Theme: Self-sufficiency")
st.write("Analyzing the balance of imports and exports to evaluate the EU's level of protein self-sufficiency.")

kpi1, kpi2, kpi3, kpi4 = st.columns(4)
kpi1.metric("Current Value", "500k Tons", "-5%")
kpi2.metric("Self-sufficiency rate", "79%", "-2%", delta_color="inverse")
...
with tab_trend:
    ...
    with left_chart:
        ...
        with chart_head:
            st.subheader(f"Annual Trend: Domestic use per feed ingredient")
        with chart_filt:
        ...
        if sub_cats:
            # 1. Generate the data (using strings for years to avoid the '2,016' comma)
            chart_data = pd.DataFrame(
                np.random.randint(100, 200, size=(10, len(sub_cats))),
                columns=sub_cats,
                index=[str(year) for year in range(2016, 2026)]
            )

            # 2. Convert from 'Wide' to 'Long' format for Altair
            chart_data_long = chart_data.reset_index().rename(columns={'index': 'Year'}).melt(
                id_vars='Year',
                var_name='Crop',
                value_name='Usage'
            )

            # 3. Create the Altair chart with horizontal labels
            line_chart = alt.Chart(chart_data_long).mark_line(point=True).encode(
                x=alt.X('Year:N',
                        axis=alt.Axis(labelAngle=0),  # 0 = Horizontal/Upright
                        title="Year"),
                y=alt.Y('Usage:Q', title="Domestic Use (Tons)"),
                color='Crop:N',
                tooltip=['Year', 'Crop', 'Usage']
    ...
    with right_chart:
        st.subheader("📊 Domestic use per crop")
